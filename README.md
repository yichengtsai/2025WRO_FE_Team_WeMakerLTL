...
- **ROS 系統應用（LiDAR 專用）** :
   ...
   - 在本專案中，ROS 僅用於處理 LDRobot D100 LiDAR 的資料，其他感測器與控制則直接由 Python 模組處理。使用 ROS (Robot Operating System) 作為 LiDAR 資料的中介層，原因是 ROS 擁有豐富的感測器驅動支援，能快速整合 LDRobot D100 LiDAR。

   - **啟動方式**：透過 ROS 套件 ldlidar 啟動 LiDAR 節-點。

   - **資料流**：LiDAR 節點以 /scan topic 發布雷射掃描數據，Python 主控程式透過 rospy 訂閱該 topic 並進行距離與角度分析。

   - **用途**：用於比賽中的車道置中與障礙物偵測，提升機器人在動態環境中的反應速度。
