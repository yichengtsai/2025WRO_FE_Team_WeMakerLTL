...
- **感測器融合演算法強化**
  ...
  - 由目前的幾何修正 + 基礎補償，提升為 EKF（Extended Kalman Filter） 或 UKF（Unscented Kalman Filter），在光達、IMU、攝影機資料融合時提高精度與抗雜訊能力。

  - 增加即時誤差估計（例如同時估算定位誤差、轉向精度）。

- **AI 視覺辨識**

  - 將 HSV 色彩遮罩升級為 深度學習物件檢測（YOLO、MobileNet-SSD），提升在不同光照與背景下的辨識準確率。

  - 透過邊緣運算（Raspberry Pi + Coral TPU）實現即時推論。

- **動態路徑規劃**

  - 從現有的避障邏輯，進一步實作 A*、D* Lite、RRT 等路徑規劃演算法，讓車輛能即時找出最佳路徑而非單純閃避。

### 2. 硬體與能源管理改進
- **高效率馬達驅動模組**

  - 改用 MOSFET 驅動 或 BLDC 無刷馬達，提高效率並減少發熱。

- **能源監控系統**

  - 增加電池管理系統（BMS） 與電壓/電流即時監控，讓控制程式可以根據剩餘電量自動調整車速或回充策略。

- **模組化底盤設計**

  - 將車體設計成可更換模組（不同輪距、不同驅動方式），方便實驗不同傳動與轉向機構。

### 3. 定位與地圖建構
- **SLAM（Simultaneous Localization and Mapping）**

  - 利用 LiDAR 與攝影機進行 2D 或 3D SLAM（例如 GMapping、Hector SLAM、ORB-SLAM2），讓車輛能在未知環境中自我定位與繪製地圖。
